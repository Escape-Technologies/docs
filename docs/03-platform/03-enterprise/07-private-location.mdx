...
The Repeater is provided as an open-source Docker image, available on [GitHub](https://github.com/Escape-Technologies/Repeater). The resource requirements depend on expected traffic; a 1 vCPU instance with 2GB RAM is typically sufficient to start, with usage monitored and scaled as necessary.

## Getting Started with Private Locations

Escape's Private Locations can be easily deployed using the Repeater Docker image, which can run on any server with access to your internal applications. You can deploy it using Docker CLI, Docker Compose, or other container orchestration tools.
