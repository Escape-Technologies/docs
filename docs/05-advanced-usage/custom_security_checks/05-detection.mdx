...
- [scan.type](#scantype)
- [schema.path_ref](#schemapath_ref)
...
The path_ref is an agnostic way to describe the action that is performed by the request.
It will match the operation in graphql, the path in REST, etc...

```yaml
detection:
  - detect: schema.path_ref
    is: "query"
    # or
    contains: "/users"
```
