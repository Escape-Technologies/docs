...
- `Environment` : The choosen mode for the tested API. `dev` mode is read-write and suited to development environment. `prod` mode is read-only and will reduce the number of tests performed and the scan coverage. Must be one of `['dev', 'prod']`.
