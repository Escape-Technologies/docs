...
- `sensitivity`: Data sensitivity level. Allowed values are `0`, `1`, `2` and 3. Values `2` and `3` will serve to raise **Sensitive Data** issues of respectively `MEDIUM` and `HIGH` sensitivity in Escape.
