...
For `graphql-yoga`, you can use the `graphql-query-complexity` library to analyze and limit the complexity of incoming queries.
...
AWS AppSync allows you to set query depth and complexity limits directly in the AWS Management Console.
...
In `graphql-ruby`, you can limit query complexity by using the query complexity analysis feature.
...
Hasura allows you to set a width (=node) limit. To do so:
