...
- [Alerting](https://docs.escape.tech/custom-tests/Alerting) is used define the alert format, it's severity…
- [Detectors](https://docs.escape.tech/custom-tests/Detectors) are used to detect if an alert must be raised by inspecting queries.
- [Transformations](https://docs.escape.tech/custom-tests/Transformations) are used to mutate the requests (optional).
- [Mutators](https://docs.escape.tech/custom-tests/Mutators) are used inside the transformations.
- [Seeders](https://docs.escape.tech/custom-tests/Seeders) are used to seed the scan with requests (optional).
...
- `seed`: A list of requests to seed the scan. See [Seeders](https://docs.escape.tech/custom-tests/Seeders)
- `transform`: Defines lists of triggers and mutations (combined with AND operators).See [Transformations](https://docs.escape.tech/custom-tests/Transformations)
- `detect`: The conditions to trigger the alert. See [Detectors](https://docs.escape.tech/custom-tests/Detectors)
- `alert`: The alert to raise if the detection conditions are met. See [Alerting](https://docs.escape.tech/custom-tests/Alerting)
