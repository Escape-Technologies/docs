...
- `SKIPPED`: The endpoint is not blacklisted, but still no operation has been sent to the endpoint.
...
Production (Read-only) scans automatically blacklist all data-mutation endpoints, depending on the API kind:
...
\[ \text{Coverage} = \frac{\text{Covered Endpoints}}{\text{Total Number of Endpoints}} \]

\[ \text{Covered Endpoints} = \text{OK} + \text{SERVER ERROR} \]

\[ \text{Total Number of Endpoints} = \text{OK} + \text{SERVER ERROR} + \text{UNAUTHORIZED} + \text{RATE LIMITED} + \text{REDIRECTED} + \text{SKIPPED} \]

So, `BLACKLISTED` and `NOT FOUND` are not taken into account in the coverage.
